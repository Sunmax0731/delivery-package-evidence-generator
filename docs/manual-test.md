...
- 作業ディレクトリ: `D:\AI\AssetPipeline\delivery-package-evidence-generator`
...
```powershell
cd D:\AI\AssetPipeline\delivery-package-evidence-generator
npm test
npm start
```
...
5. CLI + evidence manifest の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/delivery-package-evidence-generator-docs.zip` に存在する。
